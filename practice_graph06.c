...
//找距离顶点v最远的顶点
void FindFarthestVex(Graph* G, int v) {
    int distance[MAXNUM];
    int i, j, k;
    ...
    for (i = 1; i < G->vexNum; i++) {
        ...
        for (j = 0; j < G->vexNum; j++) {
            if (visited[j] == 0 && G->arcs[k][j] != INF && distance[k] + G->arcs[k][j] < distance[j]) {
    ...
    maxVertex = v;
    for (i = 0; i < G->vexNum; i++) {
        if (distance[i] > maxDist && distance[i] != INF) {
            maxDist = distance[i];
            maxVertex = i;
        }
    }
